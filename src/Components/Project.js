...
export function Project({ title, image, description, techstack, previewLink, githubLink }) {
  return (
    <article className="flex flex-col bg-white dark:bg-dark-card rounded-xl overflow-hidden shadow-xl shadow-slate-300 dark:shadow-slate-900 h-full">
      ...
      <div className="flex flex-col flex-grow p-4">
        <h1 className="text-dark-heading dark:text-light-heading font-semibold text-lg line-clamp-1">{title}</h1>
        <p className="text-content mt-2 font-light flex-grow line-clamp-3">{description}</p>
        <h3 className="text-dark-heading dark:text-light-heading font-medium mt-4 line-clamp-1">
          Tech Stack : <span className="font-light">{techstack}</span>
        </h3>
        <div className="flex justify-between items-center mt-4">
          <ProjectLink href={previewLink} text="Live Preview">
            <PreviewIcon />
          </ProjectLink>
          {title !== "Insightaction" && githubLink && (
            <ProjectLink href={githubLink} text="View Code">
              <GithubIcon />
            </ProjectLink>
          )}
        </div>
...
function ProjectLink({ href, text, children }) {
  return (
    <div className="flex items-center">
      ...
      <a
        ...
        className="underline pl-2 font-light text-dark-heading dark:text-white hover:text-blue-500 transition-colors duration-200"
      >
        {text}
      </a>
    </div>
...
function GithubIcon() {
  return (
    <svg
      className="fill-dark-heading dark:fill-white"
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
